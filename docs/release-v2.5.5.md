...
- 添加了港版 DZG1（One UI 8.5）和 CZA1（One UI 8.0，待测）适配
- 无线调试功能默认隐藏，因为还是有些问题
...
- **无线调试（实验性，默认隐藏）**：端口每次开关无线调试会变化；设置页开启后主页才显示相关控件
...
### 1. 无线调试改为设置页实验性开关（默认隐藏）

无线调试一直不稳定，本版将其收敛为**实验性功能**：

- 设置页「运行」分组新增**「无线调试授权」开关**，默认**关闭（隐藏）**
...
### 6. 港版 DZG1（One UI 8.5）与 CZA1（One UI 8.0，待测）适配

- **港版 DZG1**：新增目标支持（kernel 6.1.145 同代固件）
- **港版 CZA1**：新增目标支持（One UI 8.0, kernel 6.1.128，构建 33419968 系列），独立载荷；**待真机验证**
...
- 港版 DZG1 / CZA1 为新增目标；CZA1 待真机验证，若失败请提取该机型 boot 供定标
- 本工具仅供学习研究，请勿用于非法用途
